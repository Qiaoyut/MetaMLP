...
def get_dataset_split(path, name, train_examples_per_class, val_examples_per_class):
    ...
    if train_examples_per_class < 20:
        idx_train_ = fewshot_sample(train_examples_per_class, idx_train.view(-1), num_class, y[idx_train].view(-1))
        train_mask = torch.zeros_like(y.view(-1), dtype=torch.bool)
        train_mask[idx_train_] = True
...
def get_ogb_split(path, name):
    ...
    print('For {} dataset, we have #node: {} # feat: {} #classes: {} train_idx: {} val_inx: {} test_idx: {} u_mask: {}'
...
def fewshot_sample(per_num, train_index, num_class, train_y):
    random_state = np.random.RandomState(0)
    final_index = []
    assert per_num < 20
    for i in range(num_class):
        tmp_index = train_y == i
        tmp_index = torch.where(tmp_index == True)[0]
        assert tmp_index.shape[0] == 20
        tmp_index_ = train_index[tmp_index].data.cpu().numpy()
        train_indices = random_state.choice(tmp_index_, per_num, replace=False)
        final_index.append(train_indices)
    final_index = np.concatenate(final_index)
    final_index = torch.from_numpy(final_index)
    return final_index
