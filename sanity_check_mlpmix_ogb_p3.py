...
class SinglePolicyNet(nn.Module):
    def __init__(self, dim, policy_hidden, num_layers, dropout=0.5):
        super(SinglePolicyNet, self).__init__()
        self.dropout = dropout
        self.fc1 = nn.Linear(dim, policy_hidden)
        self.fc2 = nn.Linear(policy_hidden, 64)
        # self.fc3 = nn.Linear(64, 32)
        self.fc4 = nn.Linear(64, num_layers)
        self.norms = torch.nn.ModuleList()
        batch_norm_kwargs = {}

        for hidden_channels in [policy_hidden, 64, 32]:
...
def generate_positive(train_mask, y_mask,  y_soft_label, y_soft, pos_sample, mix_ratio):
    ...
    for i in range(c):
        ...
        num_i = c_index.shape[0]
        pos_num = int(num_i * pos_sample)
        print('# of num_i: {} total_mix: {} pos_num: {}'.format(num_i, total_mix.shape[0], pos_num))
        if total_mix.shape[0] < pos_num:
            replace_ = True
        else:
            replace_ = False
        if pos_num == 0:
        ...
        mixup_index = total_mix[np.random.choice(total_mix.shape[0], pos_num, replace=replace_), :]
...
def generate_negative(train_mask, y_mask,  y_soft_label, y_soft, neg_sample, mix_ratio):
    ...
    # total_mix = np.array([[i, j] for i in cand_index for j in cand_index if i != j])
    total_mix = np.array(list(combinations(cand_index, 2)))
    num_i = false_index.shape[0]
    pos_num = int(num_i * neg_sample)
    ...
    total_mix = total_mix[np.random.choice(total_mix.shape[0], pos_num, replace=replace_), :]
    mixup = torch.from_numpy(total_mix)
    print('### of num_i: {} total_mix: {} neg_num: {}'.format(num_i, total_mix.shape[0], pos_num))
